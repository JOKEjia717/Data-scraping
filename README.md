...
程序坚持“无法安全定位就停止”的原则：当本题引用入口缺失、列表未稳定或解析结果为空时，不会继续发送下一题。这能避免表面上完成任务、实际却把历史引用归到错误问题下。
...
- 如果首次回答结束后没有新参考入口，会通过 `reg_svg` 图标找到最新回答的重新生成控件，先点击其外层 `.flex.items-center.rounded` 打开菜单，再点击 `role="menuitem"`、文字严格为“重新生成”的长 class 菜单项。点击后 15 秒内必须检测到生成启动，否则立即停止，不会空等 5 分钟；自动重试严格限制为一次。
...
- 只抽取当前回答的直接引用子节点；
- 不把历史回答、正文或导航链接当作引用；
- 正确点击千问重新生成菜单、DeepSeek 最新“X个网页”和元宝最新来源入口；
...
2. 在 `src/platforms.ts` 中更新输入框、发送、联网搜索或通用引用入口选择器。
