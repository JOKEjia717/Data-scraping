...
单道题没有参考资料时，程序会检查最多 3 次；引用入口仍缺失、列表仍未稳定或解析结果仍为空时，只跳过当前题并继续下一题，对应平台数组保留为空。登录失效、输入框不可用或无法确认回答已经结束等会造成题目错位的严重异常，仍会停止当前平台任务。
...
- 找不到当前引用容器、列表未稳定或结果为空时最多检查 3 次，随后跳过当前题，且不会回退读取历史引用。
