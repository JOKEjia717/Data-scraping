...
const App = {
  ...
  computed: {
    ...
    /* 按 平台 → 品牌 分组（computed：同上） */
    groupedByPlatform() {
      const groups = {};
      for (const t of (this.tabTasks || [])) {
        if (!t || !t.executionId) continue; // 跳过无效任务
        const pf = t.platform || "unknown";
        if (!groups[pf]) groups[pf] = {};
        const brand = this.extractBrand(t.keyword);
        if (!groups[pf][brand]) groups[pf][brand] = [];
        groups[pf][brand].push(t);
      }
      // 按 PLATFORM_ORDER 排序平台
      const ordered = {};
      for (const pf of PLATFORM_ORDER) {
        if (groups[pf]) ordered[pf] = groups[pf];
      }
      // 剩余未知平台追加末尾
      for (const pf in groups) { if (!ordered[pf]) ordered[pf] = groups[pf]; }
      return ordered;
    },
  ...
  template: [
    ...
    '        <!-- 按 平台→品牌→问题 分组展示 -->',
    '        <div class="grouped-list" v-if="Object.keys(groupedByPlatform).length">',
    '          <div v-for="(brands, pf) in groupedByPlatform" :key="pf" class="pg-platform">',
    '            <div class="pg-pf-head">',
    '              <span class="pf-dot" :style="{ background: PLATFORM_COLORS[pf] || \'#888\' }"></span>',
    '              <span class="pg-pf-name">{{ PLATFORM_NAMES[pf] || pf }}</span>',
    '              <span class="pg-pf-count">{{ Object.values(brands).flat().length }} 条</span>',
    '            </div>',
    '            <div v-for="(tasks, brand) in brands" :key="brand" class="pg-brand">',
    '              <div class="pg-brand-head">',
    '                <span class="pg-brand-name">{{ brand }}</span>',
    '                <span class="pg-brand-count">{{ tasks.length }} 个问题</span>',
    '              </div>',
    '              <div class="pg-items">',
    '                <!-- 已运行：可展开查看详情 -->',
    '                <div v-if="selectedTab === 2" v-for="t in tasks.filter(x=>x)" :key="t.executionId"',
    '                  class="task-card done" :class="{ open: doneOpen[t.executionId] }" @click="toggleDone(t)">',
    '                  <div class="tc-top">',
    '                    <span class="tc-kw" :title="t.keyword">{{ t.keyword }}</span>',
    ...
    '                </div>',
    '                <!-- 运行中 / 待运行 / 失败：普通卡片 -->',
    '                <div v-else v-for="t in tasks.filter(x=>x)" :key="t.executionId" class="task-card" :class="selectedTab === 1 ? taskClass(t) : selectedTab === 0 ? \'pending\' : \'failed\'">',
    '                  <div class="tc-top">',
    '                    <span class="tc-kw" :title="t.keyword">{{ t.keyword }}</span>',
    '                    <span v-if="selectedTab === 1 && isStale(t)" class="badge b-red">卡住&gt;5min</span>',
    '                  </div>',
    '                  <div class="tc-meta">',
    '                    <span>ID {{ t.executionId }}</span>',
